...
export function OpportunityForm({ opportunity, mode = 'create', onSubmit, onCancel }: OpportunityFormProps) {
      ...
      <div className="space-y-6">
        <div className="rounded-[2rem] border border-slate-200/70 bg-slate-50/90 p-6 shadow-[0_24px_80px_rgba(15,23,42,0.08)] backdrop-blur-sm">
          <div className="mb-8 flex flex-col gap-4 rounded-[2rem] border border-slate-200/80 bg-white p-6 shadow-sm sm:flex-row sm:items-center sm:justify-between">
            <div>
              <p className="text-xs uppercase tracking-[0.35em] text-slate-500">Opportunity Details</p>
              <h1 className="mt-3 text-3xl font-semibold tracking-tight text-slate-950">{formState.title || 'Opportunity Preview'}</h1>
          ...
          <div className="grid gap-6 xl:grid-cols-2">
            ...
            <div className="rounded-[2rem] border border-slate-200 bg-white p-6 shadow-sm">
              <div className="mb-5 flex items-center justify-between">
                <p className="text-sm font-semibold uppercase tracking-[0.24em] text-slate-900">Notes & Context</p>
                <span className="block h-1.5 w-16 rounded-full bg-orange-400" />
              </div>
              <p className="text-sm leading-7 text-slate-700">
                This partnership opportunity represents a strategic alignment. Discussions are ongoing regarding scopes of work, milestones, and collaborative divisions. Further details will be updated as conversations mature.
              </p>
            </div>
          </div>
        </div>
          ...
          <div className="grid gap-4 xl:grid-cols-4">
            ...
            <div>
              ...
              <Input
                ...
                onChange={(e) => setFormState((current) => ({ ...current, division: e.target.value }))}
