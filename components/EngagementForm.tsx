...
export function EngagementForm({ engagement, mode = 'create', onSubmit, onCancel }: EngagementFormProps) {
  const [formState, setFormState] = useState<EngagementRecord & { followUpDate?: string; summary?: string }>(
    engagement ?? {
      id: `eng-${Date.now()}`,
      no: 0,
      type: '',
      ...
      <div className="space-y-6">
        <div className="rounded-[2rem] border border-slate-200/70 bg-slate-50/90 p-6 shadow-[0_24px_80px_rgba(15,23,42,0.08)] backdrop-blur-sm">
          <div className="mb-8 flex flex-col gap-4 rounded-[2rem] border border-slate-200/80 bg-white p-6 shadow-sm sm:flex-row sm:items-center sm:justify-between">
            <div>
              <p className="text-xs uppercase tracking-[0.35em] text-slate-500">Engagement Details</p>
              <h1 className="mt-3 text-3xl font-semibold tracking-tight text-slate-950">{formState.type || 'Engagement Preview'}</h1>
          ...
          <div className="grid gap-6 xl:grid-cols-2">
            <div className="rounded-[2rem] border border-slate-200 bg-white p-6 shadow-sm">
              ...
              <dl className="divide-y divide-slate-200 text-sm text-slate-700">
                ...
                <div className="flex justify-between py-3">
                  ...
                  <dd className="font-semibold text-slate-950">{formState.followUpDate || 'N/A'}</dd>
                </div>
              </dl>
            </div>
            ...
            <div className="rounded-[2rem] border border-slate-200 bg-white p-6 shadow-sm">
              <div className="mb-5 flex items-center justify-between">
                <p className="text-sm font-semibold uppercase tracking-[0.24em] text-slate-900">Summary & Notes</p>
                <span className="block h-1.5 w-16 rounded-full bg-orange-400" />
              </div>
              <p className="text-sm leading-7 text-slate-700">
                {formState.summary || 'No summary text registered for this engagement event. Standard processes were followed and notes will be updated during the next review meeting.'}
              </p>
            </div>
          </div>
        </div>
