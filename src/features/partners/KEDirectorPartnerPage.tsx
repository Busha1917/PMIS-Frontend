...
function DetailView({ partner, onVerify, onReject, onBack }: DetailViewProps) {
  const [verifyModalOpen, setVerifyModalOpen] = useState(false)
  const [rejectModalOpen, setRejectModalOpen] = useState(false)
  const [rejectReason, setRejectReason] = useState('')

  const canAct = partner.status === 'Pending Verification'
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Status banners */}
      {partner.status === 'Rejected' && partner.keRejectionReason && (
        <div className="rounded-xl border border-red-200 bg-red-50 p-4">
          <p className="text-sm font-semibold text-red-700">Rejection Reason</p>
          <p className="mt-1 text-sm text-red-600">{partner.keRejectionReason}</p>
        </div>
      )}
      ...
      {partner.status === 'Verified' && (
        <div className="rounded-xl border border-emerald-200 bg-emerald-50 p-4 flex items-center gap-3">
          ...
          <div>
            <p className="text-sm font-semibold text-emerald-800">Verified by KE Director</p>
            {partner.verifiedAt && (
              <p className="text-xs text-emerald-600">
                {new Date(partner.verifiedAt).toLocaleDateString()} — {partner.verifiedBy}
              </p>
            )}
          </div>
        </div>
      ...
      >
        <div className="px-6 py-6 space-y-4">
          <p className="text-sm text-slate-600">
            Verify partner <span className="font-semibold text-slate-900">{partner.id}</span>?
            <br />
            This will forward it to the Division Director for final approval.
          </p>
...
export function KEDirectorPartnerPage() {
  ...
  const handleVerify = () => {
    ...
    const updated: PartnerRecord = {
      ...selected,
      status: 'Verified',
      verifiedBy: 'KE Director',
      verifiedAt: new Date().toISOString(),
    }
    partnerStore.update(updated)
    setSelected(updated)
    toast.success('Partner verified', { description: updated.name })
  }
  ...
  const handleReject = (reason: string) => {
    ...
    const updated: PartnerRecord = {
      ...selected,
      status: 'Rejected',
      keRejectedBy: 'KE Director',
      keRejectedAt: new Date().toISOString(),
      keRejectionReason: reason,
    }
    partnerStore.update(updated)
    setSelected(updated)
  ...
  return (
    <div className="space-y-6">
      <PageHeaderCard
        ...
        subtitle="Review submitted partners and verify or reject them"
      />
      <PageToolbar
